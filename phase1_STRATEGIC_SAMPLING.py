"""
LUMIMAP Phase 1 - STRATEGIC SAMPLING (User's Brilliant Idea!)
==============================================================

Uses ALL 6 weeks but samples STRATEGICALLY:
- Ensures balanced representation across compounds
- Ensures balanced representation across MOAs
- Avoids over-sampling common compounds
- Gets more VARIETY with less total data

Expected: ~800-1000 samples with HIGH diversity
Time: 3-4 hours (faster than full 6 weeks)
Quality: Better variety than Week1-3!
"""
...
class Config:
    ...
    OUTPUT_DIR = './output/phase1_strategic'
    ...
    # ALL 6 WEEKS for variety!
    WEEKS_TO_USE = ['Week1', 'Week2', 'Week3', 'Week4', 'Week5', 'Week6']
    
    # STRATEGIC SAMPLING
    MAX_SAMPLES_PER_COMPOUND = 15  # Cap per compound for balance
    MAX_SAMPLES_PER_MOA = 100      # Cap per MOA
    
    NUM_WORKERS = 8
    USE_AMP = True
    DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
    CONCENTRATION_EMBED_DIM = 16
    NUM_CHANNELS = 3

print("="*70)
print("🎯 PHASE 1 - STRATEGIC SAMPLING (Your Brilliant Idea!)")
print("="*70)
print(f"   Weeks: ALL 6 weeks")
print(f"   Strategy: Balance across compounds & MOAs")
print(f"   Max per compound: {Config.MAX_SAMPLES_PER_COMPOUND}")
print(f"   Max per MOA: {Config.MAX_SAMPLES_PER_MOA}")
print(f"   Goal: High variety with reasonable size")
print("="*70)

def strategic_subsample(df, max_per_compound, max_per_moa):
    """
    Intelligently subsample to maximize diversity
    """
    print("\n🎯 Strategic subsampling...")
    print(f"   Input: {len(df)} total samples")
    
    # Step 1: Cap samples per compound
    sampled_indices = []
    for compound in df['Image_Metadata_Compound'].unique():
        compound_samples = df[df['Image_Metadata_Compound'] == compound]
        
        if len(compound_samples) > max_per_compound:
            # Sample randomly but ensure variety across concentrations
            selected = compound_samples.groupby('Image_Metadata_Concentration', group_keys=False).apply(
                lambda x: x.sample(min(len(x), max(1, max_per_compound // len(compound_samples.groupby('Image_Metadata_Concentration')))))
            )
            sampled_indices.extend(selected.index.tolist())
        else:
            sampled_indices.extend(compound_samples.index.tolist())
    
    df_sampled = df.loc[sampled_indices]
    print(f"   After compound balancing: {len(df_sampled)} samples")
    
    # Step 2: Check MOA balance
    print(f"\n   MOA distribution:")
    for moa in sorted(df_sampled['moa'].unique()):
        count = len(df_sampled[df_sampled['moa'] == moa])
        print(f"      {moa}: {count} samples")
    
    # Step 3: Cap per MOA if needed
    final_indices = []
    for moa in df_sampled['moa'].unique():
        moa_samples = df_sampled[df_sampled['moa'] == moa]
        
        if len(moa_samples) > max_per_moa:
            # Sample to ensure compound diversity within MOA
            selected = moa_samples.groupby('Image_Metadata_Compound', group_keys=False).apply(
                lambda x: x.sample(min(len(x), max(1, max_per_moa // len(moa_samples.groupby('Image_Metadata_Compound')))))
            )
            final_indices.extend(selected.index.tolist())
        else:
            final_indices.extend(moa_samples.index.tolist())
    
    df_final = df.loc[final_indices]
    print(f"\n   ✅ Final strategically sampled: {len(df_final)} samples")
    print(f"   Compounds represented: {df_final['Image_Metadata_Compound'].nunique()}")
    print(f"   MOAs represented: {df_final['moa'].nunique()}")
    
    return df_final

# [Same classes as before - ContrastiveDataset, encoders, etc.]
...
class MOAEncoder(nn.Module):
    def __init__(self):
        ...
        self.fusion = nn.Sequential(
            nn.Linear(Config.EMBEDDING_DIM * 3 + Config.CONCENTRATION_EMBED_DIM, 256),
            nn.ReLU(),
            nn.Dropout(0.2),
...
class Trainer:
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < self.best_val_loss:
                ...
                self.save_checkpoint('phase1_strategic_best.pth')
                print(f"✓ Best model! Loss: {val_loss:.4f}")
        
        self.plot_losses()
        print(f"\n✅ Strategic Training Complete!")
    ...
    def plot_losses(self):
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title('Phase 1 Strategic Sampling Training')
        plt.legend()
        plt.grid(True)
...
def main():
    ...
    # STRATEGIC SUBSAMPLING - Your brilliant idea!
    image_df_merged = image_df.merge(
        moa_df,
        left_on=['Image_Metadata_Compound', 'Image_Metadata_Concentration'],
        right_on=['compound', 'concentration'],
        how='inner'
    ...
    image_df_sampled = strategic_subsample(
        image_df_merged,
        Config.MAX_SAMPLES_PER_COMPOUND,
        Config.MAX_SAMPLES_PER_MOA
    )
    
    # Keep only image columns for dataset
    image_df_final = image_df_sampled[image_df.columns]
    
    dataset = ContrastiveDataset(image_df_final, moa_df, Config.DATA_DIR, transform=get_transforms())
    ...
    print("\n" + "="*70)
    print("✅ STRATEGIC SAMPLING COMPLETE!")
    print(f"   Model: {Config.OUTPUT_DIR}/phase1_strategic_best.pth")
    print(f"   High diversity, reasonable size!")
    print("="*70)
