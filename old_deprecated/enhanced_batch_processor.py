"""
LUMIMAP Enhanced Batch Processing
==================================
Complete detailed reports with:
- Resistance analysis
- Alternative treatment recommendations
- Morphology analysis (if available)
- Detailed explanations

Place images in 'input/' folder
Get organized detailed reports in 'output/' folder
"""
...
# Drug database with MOA mappings
DRUG_DATABASE = {
    'cytochalasin B': {'moa': 'Actin disruptors', 'concentration': '10 nM'},
    'latrunculin B': {'moa': 'Actin disruptors', 'concentration': '1 µM'},
    'paclitaxel': {'moa': 'Microtubule stabilizers', 'concentration': '10 µM'},
    'nocodazole': {'moa': 'Microtubule destabilizers', 'concentration': '1 µM'},
    'cisplatin': {'moa': 'DNA damage', 'concentration': '5 µM'},
    'doxorubicin': {'moa': 'DNA damage', 'concentration': '0.5 µM'},
    'DMSO': {'moa': 'DMSO', 'concentration': '0.1%'},
}

# Alternative drugs by MOA
MOA_TO_DRUGS = {
    'Actin disruptors': [
        ('cytochalasin B', 'Actin disruptors'),
        ('latrunculin B', 'Actin disruptors'),
        ('jasplakinolide', 'Actin disruptors')
    ],
    'Aurora kinase inhibitors': [
        ('hesperadin', 'Aurora kinase inhibitors'),
        ('AZ138', 'Aurora kinase inhibitors')
    ],
    'Cholesterol-lowering': [
    ...
    'Eg5 inhibitors': [
        ...
        ('S-trityl-L-cysteine', 'Eg5 inhibitors')
    ],
    'Kinase inhibitors': [
        ('staurosporine', 'Kinase inhibitors'),
        ('wortmannin', 'Kinase inhibitors')
    ],
    'Microtubule destabilizers': [
        ('nocodazole', 'Microtubule destabilizers'),
        ('colcemid', 'Microtubule destabilizers'),
...
def get_alternative_drugs(expected_moa, predicted_moa, top5_probs):
    """Generate alternative drug recommendations"""
    alternatives = []
    
    # Get all MOAs except the expected one (which failed)
    other_moas = [moa for moa, _ in top5_probs if moa != expected_moa][:5]
    
    for moa in other_moas:
        if moa in MOA_TO_DRUGS:
            # Get first drug from this MOA class
            drugs = MOA_TO_DRUGS[moa]
            if len(drugs) > 0:
                drug_name, drug_moa = drugs[0]
                alternatives.append({
                    'drug': drug_name,
                    'moa': drug_moa,
                    'score': 1.0,  # All get same score
...
# =============================================================================
# ENHANCED RESISTANCE REPORT
# =============================================================================

def generate_enhanced_resistance_report(model, image_path, drug_name, expected_moa, output_path):
    """Generate detailed resistance report with alternatives"""
    ...
    # Get top 5 for recommendations
    top5_probs, top5_indices = probabilities.topk(5)
    top5 = [(Config.MOA_CLASSES[idx.item()], prob.item()) 
    ...
    alternatives = get_alternative_drugs(expected_moa, predicted_moa, top5)
    
    # Create enhanced visualization
    fig = plt.figure(figsize=(18, 10))
    
    # Panel 1: Resistance score
    ...
    # Panel 2: Top 3 predictions
    ax2 = plt.subplot(2, 2, 2)
    moas = [moa for moa, _ in top3]
    probs = [prob*100 for _, prob in top3]
    ...
    # Panel 3: Drug information
    ax3 = plt.subplot(2, 2, 3)
    ax3.axis('off')
    
    resistance_type = 'Complete Resistance' if resistance_score > 0.7 else 'Partial Resistance' if resistance_score > 0.3 else 'Sensitive'
    
    info_text = (
        f"Drug: {drug_name}\n"
        f"Expected MOA: {expected_moa}\n"
        f"Predicted MOA: {predicted_moa}\n\n"
        f"Resistance Type: {resistance_type}\n"
    ...
    # Panel 4: Alternative recommendations
    ax4 = plt.subplot(2, 2, 4)
    ax4.axis('off')
    
    if len(alternatives) > 0:
        rec_text = "RECOMMENDED ALTERNATIVE DRUGS:\n\n"
        for i, alt in enumerate(alternatives, 1):
            rec_text += f"{i}. {alt['drug']} ({alt['moa']})\n"
            rec_text += f"   Score: {alt['score']:.2f}\n"
            rec_text += f"   Rationale: {alt['rationale']}\n\n"
    else:
        rec_text = "No alternative recommendations available"
    
    ax4.text(0.1, 0.9, rec_text, transform=ax4.transAxes,
    ...
    return {
        ...
        'top3': [(moa, float(prob)) for moa, prob in top3],
...
def process_batch(input_folder, output_folder):
    """Process all images with enhanced reports"""
    ...
    print("="*70)
    print("LUMIMAP ENHANCED BATCH PROCESSING")
    print("="*70)
    ...
    for i, image_path in enumerate(tif_files, 1):
        ...
        try:
            ...
            # Generate enhanced resistance report
            resistance_path = os.path.join(report_folder, 'resistance_report.png')
            results = generate_enhanced_resistance_report(
                model, image_path,
                DEFAULT_DRUG_NAME, DEFAULT_EXPECTED_MOA,
                resistance_path
            )
            print(f"  ✓ Generated enhanced resistance report")
            print(f"     Resistance: {results['resistance_score']*100:.2f}%")
            print(f"     Alternatives: {len(results['alternatives'])} drugs recommended")
