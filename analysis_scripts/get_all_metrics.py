...
Calculates ALL metrics and outputs them in the format:

System Performance:
- MOA Classification: XX.X% (13 classes)
- Average Confidence: 0.XXX
- Samples Evaluated: XXXX cells
- Resistance Detection: X types, XX.X% sensitive
- Best MOA: XXXX (XX.X% accuracy)
- Most Challenging: XXXX (XX.X% accuracy)
"""
...
def main():
    print("Calculating all performance metrics...\n")
    ...
    for embedding in all_embeddings:
        ...
        confidences.append(similarities[pred_moa])
    
    # Calculate overall accuracy
    overall_accuracy = accuracy_score(all_moas, predictions)
    avg_confidence = np.mean(confidences)
    
    # Calculate per-MOA accuracy
    print("Calculating per-MOA metrics...")
    true_labels = np.array(all_moas)
    pred_labels = np.array(predictions)
    
    per_moa_accuracy = {}
    for moa in unique_moas:
        mask = true_labels == moa
        if mask.sum() > 0:
            moa_acc = (true_labels[mask] == pred_labels[mask]).mean()
            per_moa_accuracy[moa] = moa_acc
    
    best_moa = max(per_moa_accuracy, key=per_moa_accuracy.get)
    worst_moa = min(per_moa_accuracy, key=per_moa_accuracy.get)
    
    # Resistance detection stats
    print("Analyzing resistance detection...")
    resistance_counts = labels_df['resistance_type'].value_counts()
    num_resistance_types = len(resistance_counts)
    total_samples = len(labels_df)
    sensitive_count = resistance_counts.get('SENSITIVE', 0)
    sensitive_pct = (sensitive_count / total_samples) * 100
    
    # Output results
    print("\n" + "="*70)
    print("LUMIMAP SYSTEM PERFORMANCE METRICS")
    print("="*70)
    print("\nCopy this for your poster/abstract:\n")
    print("-" * 70)
    print("System Performance:")
    print(f"- MOA Classification: {overall_accuracy*100:.1f}% (13 classes)")
    print(f"- Average Confidence: {avg_confidence:.3f}")
    print(f"- Samples Evaluated: {len(all_moas)} cells")
    print(f"- Resistance Detection: {num_resistance_types} types, {sensitive_pct:.1f}% sensitive")
    print(f"- Best MOA: {best_moa} ({per_moa_accuracy[best_moa]*100:.1f}% accuracy)")
    print(f"- Most Challenging: {worst_moa} ({per_moa_accuracy[worst_moa]*100:.1f}% accuracy)")
    print("-" * 70)
    ...
    print(f"  Average Confidence: {avg_confidence:.3f}")
    print(f"  Total Samples: {len(all_moas)}")
    print(f"  MOA Classes: {len(unique_moas)}")
    
    print(f"\nPER-MOA ACCURACY:")
    for moa in sorted(per_moa_accuracy.keys(), key=lambda x: per_moa_accuracy[x], reverse=True):
        print(f"  {moa:30s}: {per_moa_accuracy[moa]*100:5.1f}%")
    
    print(f"\nRESISTANCE DETECTION:")
    print(f"  Total Samples: {total_samples}")
    print(f"  Resistance Types Detected: {num_resistance_types}")
